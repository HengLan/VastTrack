...
def extract_results(results_dir, dataset, skip_missing_seq=False, plot_bin_gap=0.05, exclude_invalid_frames=False):
    """
    Args:
        results_dir - where the trackers' results store
        dataset - VastTrack class
    """
    trackers = os.listdir(results_dir)
    ...
    for seq_id, seq in enumerate(tqdm(dataset)):
        ...
        for trk_id, trk in enumerate(trackers):
            # Load results
            base_results_path = os.path.join(results_dir, trk, seq.name)
    ...
    tracker_names = [{'disp_name': t.split('_')[0]} for t in trackers]
...
def calc_seq_err_robust(pred_bb, anno_bb, target_visible=None):
